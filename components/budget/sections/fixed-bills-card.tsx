...
import { CalendarCheck2, ChevronDown, Plus, Trash2 } from "lucide-react";
...
export function FixedBillsCard({ items, onAdd, onChange, onRemove }: Props) {
  ...
  const isOpen = open || editingId !== null || askCopy;

  const handleOpenChange = (next: boolean) => {
    if ((editingId || askCopy) && !next) return;
    setOpen(next);
  };
          ...
          <div className="flex items-start justify-between gap-3">
            <CollapsibleTrigger asChild>
              <button
                ...
                aria-label={
                  isOpen ? "Recolher contas fixas" : "Expandir contas fixas"
                }
              >
